...
// WIFI
const char* SSID = "GVT-ADAA"; // SSID da Rede. Substitua aqui seu SSID de WiFi
const char* PASSWORD = "J530155416"; // Senha da Rede. Substitua aqui sua senha de WiFi
...
#define ID_MQTT  "-37.213858/-38.5853189" //id mqtt definido como as coordenada geográficas para garantir unicidade
...
void initMQTT() {
  MQTT.setServer(BROKER_MQTT, BROKER_PORT); // Informa o broker e porta utilizados, usuário e senha
}

// Função para se reconectar ao Broker MQTT caso cair a comunicação
void reconnectMQTT() {
  while (!MQTT.connected()) {
    if (MQTT.connect(ID_MQTT)) { //Aqui coloco (ID_MQTT, usuário, senha) do servidor 
    } else {
      delay(2000);
    }
  }
